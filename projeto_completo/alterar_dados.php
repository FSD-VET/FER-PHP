<?php
    ...
    function alterarDadosUsuario($nome, $email) {
        require("conexao.php");
        try {
            $sqi = "UPDATE usuarios SET nome =?, email = ? WHERE id = ?";
            $stmt = $pdo->prepare($sql);
            if ($stmt->execute([$nome, $e-mail, $_SESSION['id']]))
...
<form method="post">
    ...
    <div class="mb-3">
        <label for="" class="form-label">Emaildo usuário</label>
        <input value="<?= $usuario['email'] ?>" type="email" id="" name="" class="form-control" required="">
    </div>
